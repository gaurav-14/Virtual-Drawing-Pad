...
int main(int argc, char **argv) {
  // capture the video from camera. If laptop webcam is used, keep index = 0
  // else 1.
  VideoCapture cap(0);
  ...
  namedWindow("Display", CV_WINDOW_NORMAL); // create a window called "Display".
                                            // It displays color marker drawing
  namedWindow("Control",
              CV_WINDOW_AUTOSIZE); // create a window called "Control". Used for
                                   // thresholding
  namedWindow("color", CV_WINDOW_AUTOSIZE); // create a window called "Color".
                                            // Used for selecting colors

  // Set of Values thresholded for particular conditions changes for different
  // ligthing conditions. H - Hue, S - Saturation, V -Value of a color
  int iLowH = 0;
  int iHighH = 29;
  ...
  cvCreateTrackbar("wipeoutscreen", "color", &wipeoutscreen,
                   1);                         // Value (0 and 1)
  ...
  while (true) {
    ...
    if (dArea > 1700000) {
      cout << dArea << endl;
      line(imgLines, Point(xi, yi), Point(xf, yf), Scalar(Blue, Green, Red),
           size);
    }
